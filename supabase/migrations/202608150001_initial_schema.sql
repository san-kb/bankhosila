...
as $$
begin
  if public.current_user_role() not in ('admin','approver') then raise exception 'ไม่มีสิทธิ์อนุมัติใบลา'; end if;
  if decision not in ('approved','rejected') then raise exception 'สถานะไม่ถูกต้อง'; end if;
  update public.leave_requests set status=decision,decided_by=auth.uid(),decided_at=now(),decision_comment=decide_leave.decision_comment,updated_at=now()
  where id=request_id and status='pending';
  if not found then raise exception 'ไม่พบใบลาที่รอพิจารณา'; end if;
  insert into public.audit_logs(actor_id,action,entity_type,entity_id,details)
  values(auth.uid(),decision,'leave_request',request_id,jsonb_build_object('comment',decision_comment));
end $$;
...
using (id=auth.uid() or public.current_user_role() in ('admin','approver'));
...
using (user_id=auth.uid() or public.current_user_role() in ('admin','approver'));
...
using (user_id=auth.uid() and status='draft') with check (user_id=auth.uid() and status in ('draft','pending'));

create policy "read own attachments or approver" on public.leave_attachments for select to authenticated
using (owner_id=auth.uid() or public.current_user_role() in ('admin','approver'));
...
using(bucket_id='signatures' and owner_id=auth.uid()::text);
create policy "users and approvers read signatures" on storage.objects for select to authenticated
using(bucket_id='signatures' and (owner_id=auth.uid()::text or public.current_user_role() in ('admin','approver')));
...
using(bucket_id='leave-attachments' and (owner_id=auth.uid()::text or public.current_user_role() in ('admin','approver')));
